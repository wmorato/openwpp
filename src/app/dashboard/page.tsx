...
export default function Home() {
  ...
  // Message cache: Map<chatId, Message[]>
  ...
  useEffect(() => {
    ...
    socketInstance.on('new-message', (msg: Message) => {
      ...
      if (currentId && (msg.from === currentId || msg.to === currentId)) {
        setMessages((prev) => {
          ...
          messageCache.current.set(currentId, updated);
          return updated;
        });
        setTimeout(() => {
        ...
        const cached = messageCache.current.get(msg.chatId);
        if (cached && !cached.find(m => m.id === msg.id)) {
          messageCache.current.set(msg.chatId, [...cached, msg]);
        }
      }
    });
    ...
    socketInstance.on('chat-update', (updatedChat: Partial<Chat> & { id: string }) => {
      setChats((prevChats) => {
        ...
        if (chatIndex !== -1) {
          ...
          const newChats = [fullChat, ...chatsCopy];
          ...
          return [updatedChat as Chat, ...chatsCopy].sort((a, b) => {
            if (a.pinned && !b.pinned) return -1;
            if (!a.pinned && b.pinned) return 1;
            return (b.timestamp || 0) - (a.timestamp || 0);
          }).slice(0, 100);
    ...
    socketInstance.on('message-update', (updatedMsg: Message) => {
      setMessages((prev) => {
        ...
        if (selectedChatIdRef.current) {
          messageCache.current.set(selectedChatIdRef.current, updated);
        }
        return updated;
      });
    });
    ...
    socketInstance.on('messages', (data: { chatId: string; messages: Message[] }) => {
      if (data.chatId === selectedChatIdRef.current) {
        ...
        messageCache.current.set(data.chatId, data.messages);
        setTimeout(() => {
          if (messageContainerRef.current) {
            messageContainerRef.current.scrollTop = messageContainerRef.current.scrollHeight;
          }
  ...
  const scrollToMessage = (msgId: string) => {
    ...
    if (!element) {
      const allMessages = Array.from(document.querySelectorAll('.wa-message-row'));
      element = allMessages.find(el => el.id.includes(msgId)) as HTMLElement;
    }
